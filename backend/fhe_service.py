...
class FHEVectorSearch:
    ...
    def encrypt_vector(self, quantized: np.ndarray) -> bytes:
        """
        Encrypt a quantized vector.
        Returns serialized encrypted value for storage.
        """
        if not self._compiled:
            self.compile_circuit()

        if self.circuit is None:
            raise RuntimeError("Circuit failed to compile")

        # Encrypt using the circuit (expects both x and y inputs)
        encrypted = self.circuit.encrypt(quantized, quantized)

        # Keep a single encrypted vector for storage
        if isinstance(encrypted, (tuple, list)):
            encrypted = encrypted[0]

        if encrypted is None:
            raise RuntimeError("Encryption failed")

        # Serialize encrypted value for storage
        enc_bytes = encrypted.serialize()

        return enc_bytes

    def compute_similarity(
        self,
        encrypted_doc_bytes: bytes,
        ...
        enc_doc = Value.deserialize(encrypted_doc_bytes)
        enc_query = Value.deserialize(encrypted_query_bytes)

        # Run homomorphic computation
        encrypted_result = self.circuit.run(enc_doc, enc_query)

        # Decrypt result
        result = self.circuit.decrypt(encrypted_result)

        # Extract scalar value if result is an array or tuple
...
class FHEContext:
    ...
    def encrypt_vector(self, vector: List[float]) -> bytes:
        ...
        # Encrypt - returns serialized encrypted value
        enc_bytes = self.fhe_search.encrypt_vector(quantized)
